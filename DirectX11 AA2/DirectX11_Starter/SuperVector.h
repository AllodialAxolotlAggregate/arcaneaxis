/*--------------------------------------------------------------------------------------------------
This class made by Gabrielle Bennett in 2013. 
All rights reserved. 
--------------------------------------------------------------------------------------------------*/
...
public:
	...
	SuperVector(void): m_pData(nullptr), m_nSize(0){ };

	/* COPY CONSTRUCTOR: Make a temporary vector and use CopyContent() to copy. Then swap! */
	SuperVector(const SuperVector& other) : m_pData (nullptr), m_nSize(0)
	{
		SuperVector<T> temporal(other.m_nSize); 
	...
	void Push(const T input){
		SuperVector temporal( this->m_nSize + 1);
	...
	{
		SuperVector<T> temporal = SuperVector<T>(); // New temporary vector
		...
		// Now put temporal back onto the SuperVector
		temporary = GetTop();
	...
	{
		SuperVector<T> temporal = SuperVector<T>(); // Make a temporary vector
		T temporary;
		...
		if(index >= 0 && index <= m_nSize)
		{
			...
			// Put the temporal back onto the SuperVector: we are now at the point where we have what we need on top of the first vector.
	...
	void CopyContent(SuperVector& destination, const SuperVector& origin, const unsigned int nLimit)
		{
